...
export default async function AdminDashboardPage() {
  ...
  const [
    ...
    supabase
      .from("writer_payouts")
      .select("id, amount, destination_email, status, created_at, profiles:writer_id(display_name)")
  ...
  const totalComision = (comisionAcumulada ?? []).reduce((acc, p) => acc + Number(p.platform_fee), 0);

  const payoutsPendientes = (payouts ?? []).filter((p) => p.status === "pending" || p.status === "processing");
  const montoPendiente = payoutsPendientes.reduce((acc, p) => acc + Number(p.amount), 0);
    ...
    <div className="container mx-auto max-w-5xl px-6 py-12">
      ...
      <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <div className="rounded border border-ink/10 bg-paper-card p-5">
          ...
          <p className="mt-1 font-mono text-2xl font-semibold">{totalUsuarios ?? 0}</p>
        </div>
        <div className="rounded border border-ink/10 bg-paper-card p-5">
          <p className="text-sm text-ink-soft">Escritores activos</p>
          <p className="mt-1 font-mono text-2xl font-semibold">{totalEscritores ?? 0}</p>
        </div>
        <div className="rounded border border-ink/10 bg-paper-card p-5">
          <p className="text-sm text-ink-soft">Libros publicados</p>
          <p className="mt-1 font-mono text-2xl font-semibold">{totalLibrosPublicados ?? 0}</p>
        </div>
        <div className="rounded border border-ink/10 bg-paper-card p-5">
          <p className="text-sm text-ink-soft">Ventas totales</p>
          <p className="mt-1 font-mono text-2xl font-semibold">{totalVentas ?? 0}</p>
        </div>
      </div>

      <div className="mt-4 grid gap-4 sm:grid-cols-2">
        <div className="rounded border border-wine/20 bg-wine/5 p-5">
          <p className="text-sm text-ink-soft">Comisión acumulada de la plataforma</p>
        ...
        <div className="rounded border border-mustard/30 bg-mustard/10 p-5">
          <p className="text-sm text-ink-soft">Retiros pendientes de procesar</p>
      ...
      {payouts && payouts.length > 0 ? (
        <ul className="mt-4 divide-y divide-ink/10 rounded border border-ink/10 bg-paper-card">
          {payouts.map((payout) => {
            const autor = payout.profiles as unknown as { display_name: string } | null;
            return (
              <li key={payout.id} className="flex flex-wrap items-center justify-between gap-3 p-4">
                <div>
                  <p className="font-medium">
                    {autor?.display_name ?? "—"} · {formatearMoneda(payout.amount)}
                  </p>
                  <p className="text-sm text-ink-soft">{payout.destination_email}</p>
                </div>
                <div className="flex items-center gap-3">
                  <span className="rounded-full bg-ink/5 px-3 py-1 text-xs font-medium">
                    {ETIQUETAS_ESTADO[payout.status] ?? payout.status}
                  </span>
                  <AccionesRetiro payoutId={payout.id} estadoActual={payout.status} />
                </div>
              </li>
            );
          })}
        ...
        <p className="mt-4 text-sm text-ink-soft">Todavía no hay solicitudes de retiro.</p>
      )}
    </div>
  );
}
